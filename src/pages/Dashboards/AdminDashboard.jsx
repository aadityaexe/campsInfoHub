...
import { noticesAPI, usersAPI, coursesAPI, lostFoundAPI, assignmentsAPI } from '../../api/api';
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [usersRes, noticesRes, coursesRes, lostFoundRes, assignmentsRes] = await Promise.all([
      ...
      const pendingPosts = lostFound.filter((item) => item.status === 'pending').length;
  ...
  return (
    <div className="container-custom py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Here's what's happening in your campus</p>
      </div>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.notices}</p>
            </div>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.lostFound}</p>
            </div>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.pendingPosts}</p>
            </div>
      ...
      <Card className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Link to="/admin/notices">
            <Button variant="outline" className="w-full">Manage Notices</Button>
          </Link>
          <Link to="/admin/courses">
            <Button variant="outline" className="w-full">Manage Courses</Button>
          </Link>
          <Link to="/admin/lost-found">
            <Button variant="outline" className="w-full">Lost & Found</Button>
          </Link>
        </div>
      </Card>
      ...
      <Card className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900">Recent Lost & Found Posts</h2>
          <Link to="/admin/lost-found">
        ...
        {recentLostFound.length === 0 ? (
          ...
          <div className="space-y-4">
            {recentLostFound.map((item) => (
              <div key={item._id || item.id} className="border-b border-gray-200 pb-4 last:border-0">
      ...
      <Card className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900">Recent Assignment Uploads</h2>
        ...
        {recentAssignments.length === 0 ? (
          ...
          <div className="space-y-4">
            {recentAssignments.map((assignment) => (
              <div key={assignment._id || assignment.id} className="border-b border-gray-200 pb-4 last:border-0">
